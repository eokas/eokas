...
**公理 A5（操作与数据分离）。** 宜将操作与数据分开组织：顶层自由函数宜 `add(x, y)` 形式；Schema **成员函数**（§18）调用时绑定 `this`，宜 `instance.method(args)` 形式。函数值字段（§18）按普通字段存取，调用时按 §12.10 规则求值。
...
**自由函数风格。** 顶层自由函数及 `eokas.kernel` 预导入的堆操作 `export func`，调用形式为 `func_name(args)`（公理 A5）；Schema 成员函数调用形式为 `instance.method(args)`（§12.10）。
